...
db.transaction((tx) => {
    tx.executeSql(
        "CREATE TABLE IF NOT EXISTS menstruacoes (id INTEGER PRIMARY KEY AUTOINCREMENT, data_ultima_menstruacao TEXT, data_proxima_menstruacao TEXT, informacoes_menstruais TEXT, id_usuario INTEGER, FOREIGN KEY (id_usuario) REFERENCES usuarios(id));"
    );
});
...
const update = (id, obj) => {
    return new Promise((resolve, reject) => {
        db.transaction((tx) => {
            //comando SQL modificável
            tx.executeSql(
                "UPDATE menstruacoes SET data_ultima_menstruacao=?, data_proxima_menstruacao=?, informacoes_menstruais=?, id_usuario=? WHERE id=?;",
                [obj.data_ultima_menstruacao, obj.data_proxima_menstruacao, obj.informacoes_menstruais, obj.id_usuario, id],
                //-----------------------
                (_, { rowsAffected }) => {
